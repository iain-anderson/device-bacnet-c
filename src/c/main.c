...
static uint32_t parseInt (const iot_data_t *map, const char *name, iot_data_t **exc)
{
  ...
  if (elem == NULL)
  {
    if (*exc == NULL)
    {
      *exc = bacnet_alloc_exception ("Attribute '%s' is required", name);
    }
    return 0;
  }
...
static BACNET_PROPERTY_ID parseProperty (const iot_data_t *property, iot_data_t **exc)
{
  ...
  if (property == NULL)
  {
    if (*exc == NULL)
    {
      *exc = iot_data_alloc_string ("Attribute 'property' is required", IOT_DATA_REF);
    }
    return 0;
  } 
...
static devsdk_resource_attr_t bacnet_getattributes (void *impl, const iot_data_t *device_attr, iot_data_t **exception)
{
  ...
  attrs->instance = parseInt (device_attr, "instance", exception);
  ...
  attrs->index = parseInt (device_attr, "index", exception);
  if (*exception)
  {
    free (attrs);
    return NULL;
...
static devsdk_address_t bacnet_getaddress (void *impl, const devsdk_protocols *protocols, iot_data_t **exception)
{
  ...
  if (props)
  {
    uint32_t inst = parseInt (props, "DeviceInstance", exception);
    uint16_t port = parseInt (props, "Port", exception);
    if (exception)
    {
      return NULL;
    }
